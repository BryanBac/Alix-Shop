...
export default function VerPedidos() {
  ...
  const filtrarPedidos3 = (valorBusqueda) => {
    console.log("vB", valorBusqueda)
    console.log("AllData Filtrar 2", allData)
    return allData.filter(pedido =>{
      console.log("Productos: ", pedido.productos)
      for (let i= 0; i<pedido.productos.length; i++){
        if(pedido.productos[i].producto.toLowerCase().includes(valorBusqueda.toLowerCase())){
            return pedido.productos[i].producto.toLowerCase().includes(valorBusqueda.toLowerCase());
        }
      }
    });
  };
  ...
  const Busqueda3 = (e) => {
    const valor = e.target.value;
    if (valor != "") {
      setBuscar(filtrarPedidos3(valor));
    } else {
      const a = [...aliexpress, ...shein, ...stock]
      const ar = a.sort((a, b) => b.contador - a.contador);
      setAllData(ar)
    ...
    <>
      ...
      <div className={styles.superContainer}>
        <div className={styles.container}>
          ...
          <div className={styles.barraClientes}>
            <div className={styles.inputC}>
              <div className={styles.square1}>Buscar Producto</div>
              <div className={styles.square2}><textarea type="text" className={styles.inp} onChange={(e) => Busqueda3(e)}></textarea></div>
            </div>
          </div>
